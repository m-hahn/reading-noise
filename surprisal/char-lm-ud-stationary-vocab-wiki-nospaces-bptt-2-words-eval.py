...
# python char-lm-ud-stationary-vocab-wiki-nospaces-bptt-2-words-eval.py --batchSize 1 --char_embedding_size 200 --hidden_dim 1024 --language dailymail --layer_num 1 --load-from dailymail-words-287517088 --verbose True 
...
char_vocab_path = "vocabularies/dailymail-word-vocab-50000.txt"
...
with open("output/WORDS_"+noiseType+"_"+args.load_from+".tsv", "w") as outFile:
 ...
 for text in texts:
   ...
   numerified = torch.cuda.LongTensor([(stoi[char]+3 if char in stoi else 2) for char in sents.split(" ")]).unsqueeze(1)
...
#   print(list(zip(surprisals, sents)))
   ...
   surprisal = 0
   wordNum = 0
   for out, char in zip(surprisals, sents.split(" ")):
      surprisal += out[0]
#      if char not in ["'", '"']:
 #        word += char
...
#         print(word+" "+str(surprisal))
      print((char if char in stoi else "OOV")+"\t"+str(surprisal)+"\t"+nameText+"\t"+str(wordNum)+"\t"+args.load_from+"\t"+noiseType, file=outFile)

      print((char if char in stoi else "OOV")+"\t"+str(surprisal)+"\t"+nameText+"\t"+str(wordNum)+"\t"+args.load_from+"\t"+noiseType)
